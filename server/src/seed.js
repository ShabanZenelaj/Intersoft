...
const seed = async () => {
  ...
  const adminEmail = "admin@intersoft.al"
  ...
  if (!admins.length) {
    await query("insert into admins (email, password_hash, name) values ($1, $2, $3)", [
      adminEmail,
      await hashPassword("supersecret"),
      "Intersoft Admin",
    ])
  }
  ...
  await query(
    "insert into promotions (code, type, value) values ('WELCOME10', 'percentage', 10) on conflict do nothing"
  )
  ...
  console.log("Admin login: admin@intersoft.al / supersecret  →  http://localhost:9000/admin")
}
